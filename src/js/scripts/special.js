...
import { PROMOTIONAL, BILLING } from '../strings.js';
...
export function showClaimMembershipButton(cookieData) {
    ...
    claimButton.textContent = PROMOTIONAL.SPECIAL.CLAIM_MEMBERSHIP;
        ...
        try {
            ...
            const guardedClaim = requireAuth(async (params) => {
                ...
                if (result && result.already_active) {
                    ...
                    await prompt(BILLING.PROMPTS.ALREADY_SUBSCRIBED);
                    // Stay on landing page if already active
                } else if (result && result.error) {
                    const { prompt } = await import('/js/pages/prompt.js');
                    await prompt(BILLING.PROMPTS.CHECKOUT_ERROR(result.error));
    ...
    const updateTimer = () => {
        ...
        timerElement.textContent = PROMOTIONAL.SPECIAL.EXPIRES_IN(hours, minutes, seconds);
    };
