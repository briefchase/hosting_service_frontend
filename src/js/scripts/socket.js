...
import { SYSTEM, DEPLOY, DOMAINS } from '../strings.js';
...
export function establishWebSocketConnection(targetWebsocketPath, onOpen, onMessage, onError, onClose, statusCallback) {
    return new Promise((resolve, reject) => {
        const user = getUser();
        if (!user || !user.token) {
            if (statusCallback) statusCallback(DEPLOY.ERRORS.WS_CONNECTION_FAILED, 'error');
            reject(new Error(DOMAINS.ERRORS.AUTH_REQUIRED));
            return;
        }
        ...
        if (statusCallback) statusCallback(SYSTEM.STATUS.CONNECTING, 'info');
        ...
        ws.onerror = (event) => {
            ...
            if (statusCallback) statusCallback(SYSTEM.ERRORS.WS_ERROR, 'error');
            if (onError) onError(event);
            reject(new Error(SYSTEM.ERRORS.WS_GENERIC_ERROR));
        };
