...
export class TextScramble {
    ...
    cycle(phrases, interval) {
        ...
        const nextPhrase = () => {
            this.setText(phrases[this.currentIndex]);
...
/**
 * Applies an organic "wavering" effect to an element using CSS variables.
 * Supports independent horizontal and vertical axes.
 * @param {HTMLElement} el - The element to apply the effect to.
 * @param {Object} options - Configuration options.
 */
export function applyWaverEffect(el, options = {}) {
    if (!el) return;
    ...
    // Horizontal settings
    const varX = (Math.random() * config.variation) - (config.variation / 2);
    const durationX = config.baseDurationX + varX;
    el.style.setProperty('--waver-duration-x', `${durationX.toFixed(2)}s`);
    el.style.setProperty('--waver-delay-x', `-${(Math.random() * durationX).toFixed(2)}s`);
    el.style.setProperty('--waver-x', `${config.intensityX}px`);

    // Vertical settings
    const varY = (Math.random() * config.variation) - (config.variation / 2);
    const durationY = config.baseDurationY + varY;
    el.style.setProperty('--waver-duration-y', `${durationY.toFixed(2)}s`);
    el.style.setProperty('--waver-delay-y', `-${(Math.random() * durationY).toFixed(2)}s`);
    el.style.setProperty('--waver-y', `${config.intensityY}px`);

    // Rotation settings
    const varRotate = (Math.random() * config.variation) - (config.variation / 2);
    const durationRotate = config.baseDurationRotate + varRotate;
    el.style.setProperty('--waver-duration-rotate', `${durationRotate.toFixed(2)}s`);
    el.style.setProperty('--waver-delay-rotate', `-${(Math.random() * durationRotate).toFixed(2)}s`);
    el.style.setProperty('--waver-rotate-intensity', `${config.rotateIntensity}deg`);
    el.style.setProperty('--waver-rotate-base', `${config.tilt}deg`);

    // Global settings for single-axis waver
    const durationGlobal = (durationX + durationY + durationRotate) / 3;
    el.style.setProperty('--waver-duration', `${durationGlobal.toFixed(2)}s`);
    el.style.setProperty('--waver-delay', `-${(Math.random() * durationGlobal).toFixed(2)}s`);

    if (config.tilt) {
        el.style.setProperty('--waver-rotate', `${config.tilt}deg`);
    }
}
