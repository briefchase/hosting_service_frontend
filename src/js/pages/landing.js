...
import { PROMOTIONAL, BILLING, SYSTEM } from '../strings.js';
import { TextScramble } from '/js/scripts/effects.js';
import { hideTutorial, planToShowTutorial } from '/js/scripts/tutorial.js';
...
// Scopes centralized in /js/scripts/scopes.js; this page doesn't directly use them.

// --- State Variables ---
let taglineScramble = null;
        ...
        spotifyIframeApi.createController(spotifyEmbedEl, options, (EmbedController) => {
            ...
            const updatePlaybackState = (state) => {
                ...
                if (playPauseButtonEl) {
                    playPauseButtonEl.src = isPlaying ? '/images/pause.gif' : '/images/play.gif';
                }
            };
...
function ensureMusicControlsCreated() {
    ...
    playPauseButtonEl.src = '/images/play.gif';
    playPauseButtonEl.alt = PROMOTIONAL.ALT.PLAY_PAUSE;
...
function showTagline() {
    ...
    if (taglineEl) {
        ...
        if (!taglineScramble) {
            ...
            // Already initialized, just ensure it's visible and active.
            // We don't call cycle() again to avoid resetting to the first phrase.
        }
        requestAnimationFrame(() => {
            positionTagline();
        });
...
function ensureSpikeballCreated() {
    ...
    spikeballEl.src = '/images/spikeball.gif';
    spikeballEl.alt = PROMOTIONAL.ALT.SPIKEBALL;
...
function ensureMockupCreated() {
    ...
    mockupEl.src = '/images/clothes/froggo.png';
    mockupEl.alt = PROMOTIONAL.ALT.MOCKUP;
...
const _initiateStripeCheckout = async (params) => {
    ...
    if (!resp.ok || !data.client_secret) {
        throw new Error(data.error || BILLING.ERRORS.START_CHECKOUT_FAILED_GENERIC);
    }

    try {
        const result = await prompt(BILLING.PROMPTS.CHECKOUT_ORDER(data.client_secret));

        // If the checkout was completed successfully, show a success message
        if (result && result.status === 'answered' && result.value === 'completed') {
            await prompt(BILLING.PROMPTS.ORDER_SUCCESS);
        }
    } finally {
...
const guardedInitiateStripeCheckout = requireAuth(
    _initiateStripeCheckout,
    PROMOTIONAL.ACTIONS.PURCHASE_PRODUCT,
    { skipSubscriptionCheck: true }
);

const _checkoutProcess = async () => {
    try {
        const result = await prompt(BILLING.PROMPTS.PRODUCT_PURCHASE(
            PROMOTIONAL.LABELS.PRODUCT_TEE, 
            20, 
            '/images/clothes/froggo.png', 
            { label: PROMOTIONAL.LABELS.SIZE, values: PROMOTIONAL.LABELS.SIZES }
        ));
        ...
        const selectedSize = result.value.option;
...
function updateModeToggleImage(mode) {
    if (catModeIconEl && seriousModeIconEl && modeToggleContainerEl) {
        if (mode === 'serious') {
            ...
            modeToggleContainerEl.dataset.tooltipText = SYSTEM.LABELS.MODE.CAT;
        } else { // cat mode
            catModeIconEl.style.display = 'none';
            seriousModeIconEl.style.display = 'block';
            modeToggleContainerEl.dataset.tooltipText = SYSTEM.LABELS.MODE.SERIOUS;
        }
    }
}
...
function showModeToggle() {
    ...
    // Use requestAnimationFrame to ensure the landing-view-container is visible 
    // and its children have layout before positioning.
    requestAnimationFrame(() => {
        positionModeToggle();
    });
...
export function initialize(dayOfYear, options = {}) {
    ...
    // Refresh info button position for landing page
    refreshInfoButtonPosition();
    ...
    import('/js/scripts/cookies.js').then(async (module) => {
        const cookieData = module.check_and_cleanup_cookies();
        if (cookieData) {
            try {
                const { showClaimMembershipButton } = await import('/js/scripts/special.js');
...
export function cleanup() {
    ...
    // Refresh info button position (it will likely hide itself as landing view is gone)
    refreshInfoButtonPosition();
...
function setupLandingInterface() {
    ...
    if (consoleButton) {
        consoleButton.dataset.tooltipText = PROMOTIONAL.TOOLTIPS.VENTURE_FORTH;
    }
}
...
function initializeTaglineAnimation() {
    ...
    const phrases = PROMOTIONAL.TAGLINE_PHRASES;

    taglineScramble = new TextScramble(taglineEl, {
        idleScramble: {
            probability: 0.5,
            maxInstances: 3,
            interval: 7000,
    ...
    let cycleController = taglineScramble.cycle(phrases, 8000);

    const handleTaglineClick = () => {
        // Clicking the tagline will force a move to the next phrase immediately.
        if (cycleController) {
            cycleController.next();
        }
    };
