...
import { SYSTEM, PROMOTIONAL } from './strings.js';
...
const originalConsole = {
    ...
    // Determiner target levels
    const levelToUse = LOG_LEVELS[FRONTEND_LOG_LEVEL] ?? LOG_LEVELS.info;

    // Preserve original console methods for the app's internal use
    window.appConsole = { ...originalConsole };

    // Override console methods based on log level
    if (levelToUse > LOG_LEVELS.debug) console.debug = () => {};
    if (levelToUse > LOG_LEVELS.info) console.log = console.info = () => {};
    if (levelToUse > LOG_LEVELS.warn) console.warn = () => {};
    // console.error is always preserved

// Import the main menu initialization function from the common module
import { initializeMenu, renderMenu, cleanupCurrentMenu } from '/js/pages/menu.js';
import { getHandlers } from '/js/scripts/registry.js';
import { planToShowTutorial } from '/js/scripts/tutorial.js';
import { handleBackButtonClick, clearBackHandlers, pushBackHandler } from '/js/scripts/back.js';


// Import Stripe functions
import { initializeStripe } from '/js/menus/subscription.js';
...
} from '/js/scripts/authenticate.js';
...
import { initializeRainbowText, applyRainbowEffect, applyWaveEffect } from '/js/scripts/effects.js';
export { applyRainbowEffect, applyWaveEffect };

// Import menu configurations
import '/js/menus/dashboard.js';
// ... backup.js and others already imported below in initializeApp ...
...
async function initializeApp() {
    ...
    if (supportTextEl) {
        supportTextEl.textContent = SYSTEM.LABELS.SUPPORT.OPEN_24;
    }

    const supportNumberEl = document.querySelector('.support-number .rainbow-text');
    if (supportNumberEl) {
        supportNumberEl.textContent = SYSTEM.METADATA.PHONE;
    }

    // Update meta description
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) {
        metaDesc.setAttribute('content', SYSTEM.METADATA.DESCRIPTION);
    }

    // Update hosting tagline
    const taglineEl = document.getElementById('hosting-tagline');
    if (taglineEl) {
        taglineEl.textContent = PROMOTIONAL.LABELS.TAGLINE;
    }

    // Update continue button
    const consoleButton = document.getElementById('console-button');
    if (consoleButton) {
        consoleButton.textContent = SYSTEM.LABELS.CONTINUE;
    }

    // Update back button
    if (backButton) {
        backButton.textContent = `< ${SYSTEM.LABELS.BACK}`;
    }

    // Update footer links
    const footerLinks = document.querySelectorAll('.legal-links a');
    footerLinks.forEach(link => {
        if (link.href.includes('privacy.html')) link.textContent = SYSTEM.LABELS.FOOTER.PRIVACY;
        if (link.href.includes('tos.html')) link.textContent = SYSTEM.LABELS.FOOTER.TOS;
        if (link.href.includes('briefchase.com')) {
            const yearEl = link.querySelector('#legal-year');
            const year = yearEl ? yearEl.textContent : new Date().getFullYear();
            link.innerHTML = `© <span id="legal-year">${year}</span> ${SYSTEM.LABELS.FOOTER.BRIEFCHASE}`;
        }
        const text = link.textContent.trim().toLowerCase();
        if (text === 'code' || text === SYSTEM.LABELS.FOOTER.CODE) link.textContent = SYSTEM.LABELS.FOOTER.CODE;
        if (link.getAttribute('aria-label') === 'instagram') {
            link.setAttribute('aria-label', SYSTEM.LABELS.FOOTER.INSTAGRAM);
            const img = link.querySelector('img');
            if (img) img.alt = SYSTEM.LABELS.FOOTER.INSTAGRAM;
        }
    });

    // Update mode toggle alt text
    const catModeEl = document.getElementById('mode-toggle-cat');
    if (catModeEl) catModeEl.alt = SYSTEM.LABELS.MODE.CAT;
    const seriousModeEl = document.getElementById('mode-toggle-serious');
    if (seriousModeEl) seriousModeEl.alt = SYSTEM.LABELS.MODE.SERIOUS;
    ...
    // --- End Dynamic UI Population ---

    // Configure the auth module with a no-op redirect to ensure the user stays put after spontaneous login
    configureAuthRedirect(() => {
        console.log("[Auth] User logged in. Staying put.");
    });
    ...
    await Promise.all([
        import('/js/menus/deploy.js'),
        import('/js/menus/site.js'),
        import('/js/menus/machine.js'),
        import('/js/menus/domain.js'),
        import('/js/menus/usage.js'),
        import('/js/menus/subscription.js'),
        import('/js/menus/backup.js'),
        import('/js/menus/account.js'),
        import('/js/pages/editor.js')
    ]);
    ...
    // Trigger initial background sync if we have a user
    if (currentUser) {
        import('/js/scripts/sync.js').then(m => {
            m.triggerBackgroundSync();
        }).catch(e => console.warn("[Sync] Initial trigger failed:", e));
    }

    // If no authenticated user, attempt frictionless One Tap login
    if (!currentUser) {
        import('/js/scripts/authenticate.js').then(m => {
            // Check if we're on localhost or HTTPS to avoid 403 Forbidden on gsi/status
            const isSecure = window.location.protocol === 'https:' || window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
            if (isSecure) {
                m.initializeOneTap();
            } else {
                console.log("[Auth] One Tap skipped: Google Identity Services requires HTTPS or localhost.");
            }
        });
    }
    ...
    try {
        const manifestResp = await fetch('/images/backgrounds/manifest.json');
        if (manifestResp.ok) {
            const files = await manifestResp.json();
            if (Array.isArray(files) && files.length > 0) {
                const chosenBg = files[dayOfYear % files.length];
                backgroundUrl = `url('/images/backgrounds/${chosenBg}')`;
...
async function loadInitialView() {
    ...
    if (specialPage) {
        ...
        if (window.location.pathname === '/special') {
            try {
                const cookieModule = await import('/js/scripts/cookies.js');
        ...
        setTimeout(async () => {
            try {
                const { showWhatPrompt } = await import('/js/scripts/what.js');
...
export async function loadConsoleView(param) {
    ...
    if (getSiteMode() === 'cat') {
        import('/js/pages/landing.js').then(m => m.showMusicControls()); // Show music controls on console view
    } else {
        import('/js/pages/landing.js').then(m => m.hideMusicControls());
    }
    ...
    if (accountButton && !accountButton.dataset.listenerAdded) {
        ...
        accountButton.addEventListener('click', () => {
            // Respect the global action lock if it exists
            const siteContainer = document.getElementById('console-container');
            if (siteContainer && siteContainer.dataset.actionInFlight === 'true') {
                console.log("[Main] Auth action already in flight, ignoring click.");
                return;
            }

            if (accountButton.textContent === SYSTEM.LABELS.AUTHENTICATE) {
                ...
                if (statusContainerForAuth) {
                    ...
                    updateStatusDisplay(SYSTEM.STATUS.AUTH_ELEMENT_MISSING, "error"); // Use the imported updateStatusDisplay
                }
            }
        });
    }
    ...
    if (!menuContainerElement) {
        ...
        updateStatusDisplay(SYSTEM.STATUS.ERROR_MENU, "error"); // Use the imported updateStatusDisplay
        return;
    }
    ...
    if (param && typeof param === 'object' && param.specialNav === 'viewSite') {
        if (param.id) {
            console.log(`[Main] Executing specialNav: viewSite for ${param.id}`);
            const { viewSite } = await import('/js/menus/site.js');
            await viewSite({ id: param.id });
        }
        console.log("Console view loaded and initialized (Special Nav).");
        return; 
    }
...
export async function loadLandingView(options = {}) {
    ...
    // Hide account button on landing
    ...
    const landingModule = await import('/js/pages/landing.js');
...
export async function fetchWithAuth(url, options = {}) {
    ...
    if (response.status === 401) {
        ...
        console.log("[Auth] Session expired (401), clearing state.");
        try { sessionStorage.removeItem('currentUser'); } catch (_) {}
        currentUser = null;

        // If silent is true, we return null instead of throwing ReauthInitiated
        if (options.silent) {
            return null;
        }
    ...
    if (response.status === 403 || response.status === 412) {
        // If silent is true, we return null instead of throwing error
        if (options.silent) {
            return null;
        }

        // Forbidden or Precondition Failed: Likely a subscription or initialization issue.
        // We parse the body to get the specific error ID for the guards.
        let data = {};
        try {
        ...
        const error = new Error(data.message || data.error || SYSTEM.ERRORS.HTTP_ERROR(response.status));
        error.id = data.error;
...
export function updateAccountButtonVisibility(isVisible, isAuthenticated = false) {
    if (accountButton) {
        if (isVisible) {
            ...
            accountButton.style.pointerEvents = ''; // Reset pointer events
            accountButton.style.opacity = '';      // Reset opacity
            // Update text and data attribute based on authentication status.
            if (isAuthenticated) {
                accountButton.textContent = SYSTEM.LABELS.ACCOUNT;
                accountButton.dataset.targetMenu = 'account-menu';
            } else {
                accountButton.textContent = SYSTEM.LABELS.AUTHENTICATE;
...
/**
 * Sets the account button to a loading state during One Tap.
 */
export function setAccountButtonLoading(isLoading) {
    if (!accountButton) return;
    if (isLoading) {
        accountButton.textContent = SYSTEM.LABELS.AUTHENTICATING;
        accountButton.style.pointerEvents = 'none'; // Disable clicks
        accountButton.style.opacity = '0.7';        // Dim slightly
    } else {
        // Resetting is handled by updateAccountButtonVisibility()
    }
}
